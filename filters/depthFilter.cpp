...
static void fillOcclusionInv_(Mat& src, Mat& image, const T invalidvalue, const T minval, int threshold)
{
	...
	for(int j=bb;j<src.rows-bb;j++) // Per tutte le righe dell'immagine
	{
		T* s = src.ptr<T>(j); // puntatore a pixels depth a partire dal pixel j
		uchar *imap = image.ptr<uchar>(j); // puntatore a pixels rgb a partire dal pixel j

		s[0]=minval; // Metto il primo valore del pixel a minval(=0)
		s[src.cols - 1] = minval; // Metto il valore dell'ultimo pixel a minval(=0)
		for(int i=1;i<src.cols-1;i++) // per ogni colonna quindi scorro i pixel della riga jesima
		{
			if(s[i]<=invalidvalue) 
			{
				int t=i;
				...
				{
					const int kstep = 2;
					// Faccio la differenza tra i pixel sempre più avanti
					// e mi calcolo la differenza massima sui pixel dell'immagine RGB
					int E = 
						max(max(
				...
				}while(s[t]<=invalidvalue); // Finchè l'immagin edepth in t è invalida
...
// This function provide a simple noise reduction not based on a box but on stripes
// It looks into the column pixels difference in oreder to to make pixel with small variation 
// less noisy. In pratica si guarda se la differenza tra un pixel e uno sotto (o due)(quindi cambio riga) è minore di una certa threashold.
// In tal caso significa che non c'è un dislivello, quindi faccio la media. Se la differenza tra il pixel sulla colonna accanto e la media 
// appena fatta supera la soglia allora sostituisco con la media.
	...
	if(src.type()==CV_16S)
	{
		...
		for(j=bb;j<src.rows-bb-1;j++)
		{
			...
			for(i=0;i<src.cols;i++)
			{
				...
				{
					// Modo bellissimo per fare diviso due
